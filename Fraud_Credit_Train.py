...
def run_experiment(config, x_train, y_train, x_val, y_val, scaler_choice=None):
    scaler = None
    if config["scaler"]:
        # Use the scaler_choice parameter if provided, otherwise use config scaler
        scaler_to_use = scaler_choice if scaler_choice else config["scaler"]
        x_train, x_val, _, scaler = apply_scaling(x_train, x_val, None, scaler_to_use)
...
def main():
    ...
    parser.add_argument('--scaler', type=str, default="standard",
                        ...
                        help='Scaler to use: standard, minmax, robust, or none')
    ...
    EXPERIMENTS = [
        ...
        {"scaler": args.scaler if args.scaler != "none" else None, "sampling": None, "cost_sensitive": False,
         "include_mlp": True, "name": "scaled"},
        {"scaler": None, "sampling": "undersample", "cost_sensitive": False, "include_mlp": False,
         "name": "undersample"},
        {"scaler": args.scaler if args.scaler != "none" else None, "sampling": "undersample", "cost_sensitive": False,
         "include_mlp": True, "name": "undersample_scaled"},
        {"scaler": None, "sampling": "oversample", "cost_sensitive": False, "include_mlp": False, "name": "oversample"},
        {"scaler": args.scaler if args.scaler != "none" else None, "sampling": "oversample", "cost_sensitive": False,
         "include_mlp": True, "name": "oversample_scaled"},
        {"scaler": None, "sampling": "over_and_under", "cost_sensitive": False, "include_mlp": False,
         "name": "over_and_under"},
        {"scaler": args.scaler if args.scaler != "none" else None, "sampling": "over_and_under",
         "cost_sensitive": False, "include_mlp": True, "name": "over_and_under_scaled"},
        {"scaler": None, "sampling": None, "cost_sensitive": True, "include_mlp": False, "name": "cost_sensitive"},
        {"scaler": args.scaler if args.scaler != "none" else None, "sampling": None, "cost_sensitive": True,
         "include_mlp": True, "name": "cost_sensitive_scaled"},
    ]
    ...
    if args.run == "all":
        for config in EXPERIMENTS:
            ...
            run_experiment(config, x_train.copy(), y_train.copy(), x_val.copy(), y_val.copy(),
                           args.scaler if args.scaler != "none" else None)
        ...
        run_experiment(best_config, x_train, y_train, x_val, y_val, args.scaler if args.scaler != "none" else None)
    else:
